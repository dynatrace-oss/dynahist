 ...
 * i is an integer.
 *
 * <p>Zero values have its own bucket with index 0. For positive values the smaller bound is exclus
 * and the larger bound is inclusive. Positive bins are sequentially numbered starting with 1. Bins
 ...
 *
 * <p>This layout corresponds to the layout of OpenTelemetry exponential histograms (see <a
 * href="https://opentelemetry.io/docs/specs/otel/metrics/data-model/#exponentialhistogram">OpenTelemetry
 * specification</a>). Values recorded using this layout can be losslessly transformed into
 * OpenTelemetry exponential histograms with corresponding scale parameter and a zero threshold of
 * 0.
 *
 ...
 */
public final class ExponentialHistogramLargeInclusiveLayout
    extends AbstractExponentialHistogramLayout {

  private static final byte SERIAL_VERSION_V0 = 0;

  private static final AtomicReferenceArray<ExponentialHistogramLargeInclusiveLayout> INSTANCES =
   ...
   * @return a new {@link ExponentialHistogramLargeInclusiveLayout} instance
   */
  public static ExponentialHistogramLargeInclusiveLayout create(int scale) {
    ...
    return INSTANCES.updateAndGet(
        ...
        x -> {
          if (x != null) {
            ...
            return new ExponentialHistogramLargeInclusiveLayout(scale);
          }
        });
  }

  ExponentialHistogramLargeInclusiveLayout(int scale) {
    super(scale);
  ...
  private static int mapToBinIndexHelper(
    ...
    if (mantissa == 0 && exponent != 0x7ff) k -= 1;
   ...
   * @throws IOException if an I/O error occurs
   */
  public static ExponentialHistogramLargeInclusiveLayout read(DataInput dataInput)
      throws IOException {
    checkSerialVersion(SERIAL_VERSION_V0, dataInput.readByte());
    int tmpScale = dataInput.readUnsignedByte();
    return ExponentialHistogramLargeInclusiveLayout.create(tmpScale);
  }

  @Override
  public boolean equals(Object o) {
    if (this == o) return true;
    if (!(o instanceof ExponentialHistogramLargeInclusiveLayout)) return false;
    ExponentialHistogramLargeInclusiveLayout that = (ExponentialHistogramLargeInclusiveLayout) o;
